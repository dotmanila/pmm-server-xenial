...
def start_grafana():
    res = None
    if os.path.exists('/usr/local/bin/supervisorctl'):
        res = subprocess.call(["/usr/local/bin/supervisorctl", "start", "grafana"])
    else:
...
def stop_grafana():
    res = None
    if os.path.exists('/usr/local/bin/supervisorctl'):
        res = subprocess.call(["/usr/local/bin/supervisorctl", "stop", "grafana"])
    else:
